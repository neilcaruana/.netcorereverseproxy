...
namespace ReverseProxyServer.Core
{
    public class ReverseProxy
    {
        ...
        public IEnumerable<Statistics> Statistics => [.. statistics];
        ...
        private readonly ConcurrentBag<Statistics> statistics = [];
        private readonly List<EndpointConfig> settings;
        private readonly List<Task> listeners = [];
        private Task? cleanPendingConnections;
        private readonly CancellationTokenSource cancellationTokenSource = new();
        public ReverseProxy(List<EndpointConfig> reverseProxySettings, CancellationTokenSource cancellationTokenSource)
        {
        ...
        public void Start()
        {
            foreach (EndpointConfig endpointSetting in settings)
            {
        ...
        private async Task CreateEndpointListener(int port, EndpointConfig endpointSetting, CancellationToken cancellationToken)
        {
            TcpListener? listener = null;
            ILogger mainLogger = new Logger(endpointSetting.LoggerType, endpointSetting.LoggerLevel, endpointSetting.ProxyType.ToString());
            ILogger rawDataLogger = new Logger(LoggerType.File, LoggerLevel.Debug, "", "rawdata");
            try
            {
        ...
        private async Task ProxyTraffic(TcpClient incomingTcpClient, EndpointConfig endpointSetting,
                                        ILogger endpointLogger, ILogger rawDataLogger, string sessionId, 
        ...
        private async Task RelayDataAsync(NetworkStream inputStream, NetworkStream outputStream, ILogger endpointLogger, ILogger rawDataLogger, 
        ...
        private async Task<MemoryStream> ConvertNetworkStreamIntoMemory(NetworkStream networkStream, ILogger endpointLogger, string sessionId, CancellationToken cancellationToken)
        {
        ...
        private void ProcessStatistics(TcpClient newConnection)
        {
            if (newConnection.Client.LocalEndPoint is IPEndPoint local && newConnection.Client.RemoteEndPoint is IPEndPoint remote)
                statistics.Add(new Statistics(DateTime.Now, local.Address.ToString(), local.Port, remote.Address.ToString(), remote.Port));
        }
        private async Task CleanCompletedConnectionsAsync(ILogger logger, CancellationToken cancellationToken)
        {
            try
            {
                while (!cancellationToken.IsCancellationRequested)
